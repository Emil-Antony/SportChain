...
const Home: React.FC = () => {
    ...
    const getConnectedAccount = async () => {
      ...
      try {
        ...
        if (accounts.length > 0) {
          ...
          // Sign the message only if the chain check was successful
          if (success) {
            setAccount(useracc);
            goDash(); // Set account only after successful signing 
          }
          else{
            disconnectMetaMask();
          }
  ...
  const checkChain = async () => {
    ...
    console.log("Current chain ID:", chainId); // Log current chain ID
    if (chainId !== "0x13882") {
  ...
  useEffect(() => {
    ...
    const handleChainChanged = async () => {
      ...
      console.log("Chain changed to:", chainId); // Log chain change
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black-100">
      <h1 className="text-2xl text-white mb-4">Welcome to <span className="font-extrabold text-transparent text-3xl sm:text-4xl xl:text-5xl bg-clip-text bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 bg-transparent h-full">SportChain</span></h1>
      {isMetaMaskInstalled ? (
        <>
          {account ? (
            ...
            <button type="button" onClick={connectMetaMask} class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-3 mt-4 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 me-2 mb-2">
